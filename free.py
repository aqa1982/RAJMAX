...
class app:
    def __init__(self, token, id):
        self.token = token
        self.id = id
        self.erorr = "username_is_taken"

    def file_user(self, name_file):
        try:
    ...
    def SendBot(self, token, id, username):
        requests.post(
            f'https://api.telegram.org/bot{token}/sendMessage?chat_id={id}&text=⌯  تم صيد يوزر\n⚋⚋⚋⚋⚋⚋(@PPGBB)⚋⚋⚋⚋⚋⚋ \n⌯ ᴜѕᴇʀɴᴀᴍᴇ : @{username} \n⚋⚋⚋⚋⚋⚋(@PPGBB)⚋⚋⚋⚋⚋⚋\n😎 Tele : @bsx_h2')

    def random_user(self, number):
        while True:
            try:
                user1 = str("".join(random.choice("qwertyuiopasdfghjklzxcvbnm0987654321_.") for i in range(5)))
                us = str("".join(random.choice("_" + user1) for i in range(int(number))))
                self.run(us)
            except:
                pass

    def run(self, username):
        respons = requests.post("https://www.instagram.com/accounts/web_create_ajax/attempt/",
                                headers={'Host': 'www.instagram.com', 'content-length': '70',
                                         'sec-ch-ua': '"Chromium";v="106",', 'x-ig-app-id': '936619743392459',
                                         'x-ig-www-claim': 'hmac.AR3-jjxWxD7osd7WtAM_c6iLQHDvlDZucbWWVQ38t2tPdF3t',
                                         'sec-ch-ua-mobile': '?0', 'x-instagram-ajax': 'de83d85cf90c',
                                         'user-agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/106.0.0.0 Safari/537.36',
                                         'viewport-width': '980', 'content-type': 'application/x-www-form-urlencoded',
                                         'accept': '*/*', 'x-requested-with': 'XMLHttpRequest', 'x-asbd-id': '198387',
                                         'x-csrftoken': 'NEhRd2uaYf6774IYrUuw6svzofnU3COW',
                                         'sec-ch-prefers-color-scheme': 'dark', 'sec-ch-ua-platform': '"Linux"',
                                ...
                                data={"email": "email=a@gmail.com+", "username": username, "first_name": "ruks",
                                      "opt_into_one_tap": "false"}).text

        if self.erorr in respons:
            print(f'\033[1;33m[\033[1;31m*\033[1;33m] Unavailable\033[1;31m - [{username}]')
        else:
            print(f'\033[1;33m[\033[1;31m*\033[1;33m] Available\033[1;32m - [{username}]')
            threading.Thread(target=self.SendBot, args=(self.token, self.id, username)).start()
    ...
    for BL in fie:
        if ('@gmail.com') in BL:
            ...
            try:
                ...
                system('clear')
                print('الانترنيت ضعيف اقترب من الانترنيت')
                continue
            ...
            if ('"invalid_user"') in re:
                ...
                ma += 1
                print(
                    f'\033[1;32m Available Gmail : {L}\n\033[1;33m Bad gmail : {bd} \n\033[1;33m Bad insatgram : {ma} \n\033[1;36m——————————————×——————————————\n\033[1;32mTelegram = @PPGBB\nChecker mail = 『 {BL} 』')
            ################$##########
...
def followers():
    ...
    for followee in profile.get_followers():
        ...
        name3 = name2.split(' (')[0]
        follow_list.append(name3 + '@gmail.com')
        file = open("gmail.txt", "a")
...
def resa():
    email = input(' \033[1;36mENTAR EMAIL 🔺\033[1;32m ')
    check = Hamody.Reset(email)
    if check == True:
        print("\033[2;32m•  تم توصيل الريست •")
    else:
...
def insm():
    ...
    res = requests.post('https://i.instagram.com/api/v1/accounts/send_password_reset/', headers=headers,
                        data=data).json()
    rs = str(res['obfuscated_email'])
    lo = (f'''
{F}
...
''')
    print(lo)
    print(' تم ارسال معلومات لبوت التليجرام         ')
    tlg = (f"""
━━━━━━━━⧪━━━━━━━━
اسم الحساب » {nam}
يوزر الحساب » @{user}
...
if H == 6:
    ...
    id = input(f'\033[1;33m[\033[1;31m⌯\033[1;33m] Id bot :\033[1;36m')
    token = input(f'\033[1;33m[\033[1;31m⌯\033[1;33m] Token bot :\033[1;36m')
    Len = number = input(f'\033[1;33m[\033[1;31m⌯\033[1;33m] USERS insta - صيد عشوائي  :\033[1;36m')
    print("=" * 20)
    app(token, id).random_user(Len)
    exit()
elif H == 11:
    name_file = number = input(f'\033[1;33m[\033[1;31m⌯\033[1;33m] Enter Name File - ارسل اسم الملف  :\033[1;36m')
    print("=" * 20)
    app(token, id).file_user(name_file)
    exit()
elif H == 12:
    sison()
    exit()
